...
generate_ehon_from_samples ツールを必ず使い、bookCore (物語の核) と pages (各ページ) の両方を出力してください。bookCore は読者には見せませんが、これを最初に決めることで物語の整合性が保たれます。"""
    ...
    let private buildUserMessageChaos (pageCount: int) (catalogText: string) =
        let content =
            sprintf
                 ...
                 **【大事な変更】**\n\
                 20 枚の画像から **8〜12 枚を選抜** してください。全部使う必要はありません。\n\
                 選抜したら、そのうち **主役を 1-2 人** 決めて、物語の中心に据えてください。\n\
                 残りの登場人物はサポート役 (出会う人、通りすがり、助ける人、対立する人) として配置します。\n\n\
                 **【深い物語にする】**\n\
                 「登場人物全員出演、以上」ではなく、**主役の感情や行動を追う** 物語にしてください。\n\
                 - 主役が何を思ったか、何をしたか、どう変わったか\n\
                 - 他の登場人物との出会いがどう主役に影響したか\n\
                 - ただ並べるのではなく、時間の流れと関係の変化を描く\n\n\
                 **【画像選抜の順序 — 必ずこの順で考えること】**\n\n\
                 1. **まずカタログをざっと眺める** (個々の画像に深入りしない)\n\
                 2. **物語の核を決める** (bookCore の primalTheme / structure / twistMoment を頭の中で組む)\n\
                    - 例: 「箱の中に何かがいる」「鬱な日常からの脱出」「失くしたものを探す」\n\
                 3. **物語に必要な 8-12 枚を選抜する** (物語を先に決めて、それを支える画像を後から選ぶ)\n\
                 4. **選ばなかった画像は存在しないものとして扱う** — どんなに面白い画像でも、物語に馴染まないなら捨てる\n\n\
                 **【画像選抜の厳格なルール】**\n\n\
                 - **世界観が合わない画像は絶対に使わない**\n\
                   - 悪い例: 「宇宙人との出会い」の物語に「イチゴオレ」「ゲーム機」「保険証」\n\
                   - 良い例: 「宇宙人との出会い」の物語には「空」「光」「草原」「窓」「手」など\n\
                 - **画像同士が同じ空気を持っているかチェック** — 並べて違和感がないか\n\
                 - **使わない画像が 10-15 枚あっても全く問題ない** — むしろ正常\n\
                 - **各ページの画像は、そのページのテキストと直接関係する** — テキストで触れない画像を飾りで置かない\n\n\
                 **【禁止例 — 物語破壊の典型】**\n\n\
                 物語が「静かな喪失」なのに、途中で「ドバシャ！いちごオレがこぼれた」のような明るい日用品が突然登場するのは絶対ダメ。世界観を壊すだけの「便利な画像」は、どんなに残り画像が少なくても使わない。\n\n\
                 **【文章の長さ】**\n\
                 1 ページあたり **1〜2 文、全体で 10〜22 文字** まで OK。\n\
                 ただし冗長にはしない。短くても深い表現を優先。\n\n\
                 **【残す要素】**\n\
                 - ひらがな 95%% 以上、わかちがき、5 歳が読める言葉\n\
                 - 反復・リズム・対比のいずれか\n\
                 - 説明しない、見せる\n\
                 - 結末は答えを与えない\n\
                 - 道徳的フレーズ / 締め言葉の禁止\n\n\
    ...
    let private validateChaosCoverage
        ...
        let used = Set.count (Set.difference expected (Set.ofList missing))

        if List.isEmpty missing then
            Ok story
        else
            // Loop 4 方針転換: chaos でも画像を選抜するため、未使用画像を許容する。
            // cameo 自動注入は物語の整合性を損なうため廃止。warning log のみ残す。
            printfn "[chaos-coverage] used %d/%d imageIds (unused: %d — this is normal under the Loop 4 selection policy)" used samples.Length missing.Length
            Ok story
