<script lang="ts">
	...
	let visualMode = $derived<EhonMode>(nazenaze.story?.mode === 'false' ? 'chaos' : 'cosmos');
...
{#if nazenaze.story}
	<div class="viewer-wrap" data-mode={visualMode}>
		<h2 class="book-title" data-mode={visualMode}>{nazenaze.story.title}</h2>
		...
		<div class="controls">
			<button
				class="ctrl-btn prev"
				onclick={goPrev}
				disabled={nazenaze.currentPage === 0}
				aria-label="まえのページ"
			>
				&larr; まえへ
			</button>
			<span class="indicator">
				{nazenaze.currentPage + 1} / {nazenaze.totalPages}
			</span>
			{#if nazenaze.isLastPage}
		...
		<button class="back-home" onclick={() => nazenaze.exit()}>&times; ほーむへ</button>
	</div>
{/if}
